...
<div class="max-w-5xl space-y-5">
    ...
    <div>
        ...
        <h1 class="mt-3 text-2xl font-extrabold tracking-tight text-base-content">Tambah Lokasi</h1>
        <p class="mt-1 text-sm text-muted">Cari wilayah atau gambar manual area polygon geofencing</p>
    </div>
    ...
    <div class="k3l-card-static">
        <div class="p-5 lg:p-7">
            <form action="{{ route('lokasi.store') }}" method="POST" class="space-y-5" id="lokasiForm">
                @csrf

                <div class="form-control">
                    <label class="label py-1"><span class="label-text text-xs font-bold">Nama Lokasi</span></label>
                    <input type="text" name="nama_lokasi" id="namaLokasi" value="{{ old('nama_lokasi') }}" class="input input-bordered input-sm w-full" placeholder="Contoh: Kantor K3L" required>
                </div>

                {{-- Search --}}
                <div class="form-control">
                    <label class="label py-1"><span class="label-text text-xs font-bold">Cari Batas Wilayah</span></label>
                    <p class="mb-2 text-xs text-muted">Ketik nama wilayah (kecamatan/kelurahan/kota) lalu klik Cari.</p>
                ...
                <div>
                    ...
                    <p class="mb-2 text-xs text-muted">Polygon bisa otomatis dari pencarian, atau gambar manual dengan tombol di kiri atas peta.</p>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="form-control">
                        ...
                        <input type="text" name="latitude" id="latitude" value="{{ old('latitude') }}" class="input input-bordered input-sm bg-base-200" readonly>
                    </div>
                    <div class="form-control">
                        <label class="label py-1"><span class="label-text text-xs font-bold">Longitude (Pusat)</span></label>
                        <input type="text" name="longitude" id="longitude" value="{{ old('longitude') }}" class="input input-bordered input-sm bg-base-200" readonly>
                    </div>
                    <div class="form-control">
                        <label class="label py-1"><span class="label-text text-xs font-bold">Radius Fallback (m)</span></label>
                        <input type="number" name="radius" value="{{ old('radius', 100) }}" class="input input-bordered input-sm w-full" placeholder="100">
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function searchBoundary() {
        ...
        try {
            ...
            if (!data.length) { searchResults.innerHTML = '<div class="text-xs font-semibold text-error">Wilayah tidak ditemukan. Coba kata kunci lain.</div>'; return; }
            ...
            document.querySelectorAll('.search-result-btn').forEach(btn => {
                btn.addEventListener('click', async function() {
                    ...
                    if (item.hasPolygon) { applyGeoJson(item.geojson, item.display_name); searchResults.innerHTML = '<div class="text-xs font-semibold text-success">Batas wilayah berhasil diterapkan ke peta.</div>'; }
    ...
    async function fetchBoundaryFromOverpass(osmId, displayName) {
        try {
            ...
            searchResults.innerHTML = '<div class="text-xs font-semibold text-success">Batas wilayah berhasil diterapkan ke peta.</div>';
            return true;
    ...
    function assemblePolygon(ways) {
        ...
        while (segments.length > 0 && maxIterations > 0) {
            ...
            for (let i = 0; i < segments.length; i++) {
                const seg = segments[i]; const segFirst = seg[0]; const segLast = seg[seg.length - 1];
                if (lastPoint[0] === segFirst[0] && lastPoint[1] === segFirst[1]) { result.push(...seg.slice(1)); segments.splice(i, 1); found = true; break; }
                else if (lastPoint[0] === segLast[0] && lastPoint[1] === segLast[1]) { result.push(...seg.reverse().slice(1)); segments.splice(i, 1); found = true; break; }
            }
            if (!found) result.push(...segments.shift());
        }
        if (result.length > 1) { const first = result[0]; const last = result[result.length - 1]; if (first[0] === last[0] && first[1] === last[1]) result.pop(); }
        return result;
    }
