...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    ...
    <article class="surface-card p-5">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Titik absensi dan area kerja terdaftar.</p>
...
if (polygon && polygon.length >= 3) {
    L.polygon(polygon.map(p => [p[0], p[1]]), { color: '#0284C7', fillColor: '#7DD3FC', fillOpacity: 0.2 }).addTo(map).bindPopup('Area Kerja');
} else if (fenceLat && fenceLng) {
    L.circle([fenceLat, fenceLng], { radius: radius, color: '#0284C7', fillColor: '#7DD3FC', fillOpacity: 0.2 }).addTo(map).bindPopup('Area Kerja');
}
